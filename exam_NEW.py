...
gbt_CC = ensemble.GradientBoostingClassifier(
                                             n_estimators=n_estimators_optimal_CC, # We found the optiamlly n_estimators to be: 10
                                             min_samples_split=min_samples_split_optimal_CC, # We found the optimally min_samples_split to be: 2
                                             min_samples_leaf=min_samples_leaf_optimal_CC, # We found the optimally min_samples_leaf to be: 10
                                             learning_rate=learning_rate_opt_CC) # We found the optimally learning_rate to be: 0.1
...
gbt_D = ensemble.GradientBoostingClassifier(
                                             n_estimators=n_estimators_optimal_D, # We found the optiamlly n_estimators to be: 20
                                             min_samples_split=min_samples_split_optimal_D, # We found the optimally min_samples_split to be: 5
                                             min_samples_leaf=min_samples_leaf_optimal_D, # We found the optimally min_samples_leaf to be: 10
                                             learning_rate=learning_rate_opt_D) # We found the optimally learning_rate to be: 0.1
...
gbt_Y = ensemble.GradientBoostingClassifier(
                                             n_estimators=n_estimators_optimal_Y, # We found the optiamlly n_estimators to be: 20
                                             min_samples_split=min_samples_split_optimal_Y, # We found the optimally min_samples_split to be: 5
                                             min_samples_leaf=min_samples_leaf_optimal_Y, # We found the optimally min_samples_leaf to be: 10
                                             learning_rate=learning_rate_opt_Y) # We found the optimally learning_rate to be: 0.1
...
#%% Calculateing the Charecter accuraccy and sequence accuracy for the final model
...
# Get a list of whith boolean values where True is if the prediction is correct, and False otherwise.
p1=(y_test_hat_CC==y_test_CC)
p2=(y_test_hat_D==y_test_D)
p3=(y_test_hat_Y==y_test_Y)

# Calculate the points for each model
...
# If all three sub models classifie the picture correctly, the Sequence accuracy increases by 1.
for i in range(len(p1)):
    if p1[i] == p2[i] == p3[i] == True:
       p_sequence = p_sequence + 1

# For charecter accuracy every sub model gets 1/3 of a point for each correct classification
p_charecter = round(p_CC*0.33 + p_D*0.33 + p_Y*0.33)
    
print('point for CC-model:', round(p_CC), 'out of', len(y_test_CC))
print('point for D-model:', round(p_D), 'out of', len(y_test_CC))
print('point for Y-model:', round(p_Y), 'out of', len(y_test_CC))
print('point for character acc:', p_charecter, 'out of', len(y_test_CC))
