...
import numpy as np
# FOR CC
X_train, y_train = next(train_generator_CC)
real_labels_CC = y_train
real_labels_CC=real_labels_CC.astype(int)
predictions = model_CC.predict(X_train)
print(predictions)
predict_labels_CC = np.argmax(predictions, axis=-1)
print('predict for CC',predict_labels_CC)

# FOR D
X_train, y_train = next(train_generator_D)
real_labels_D = y_train
real_labels_D=real_labels_D.astype(int)
predictions = model_D.predict(X_train)
print(predictions)
predict_labels_D = np.argmax(predictions, axis=-1)
print('predict for D',predict_labels_D)

# FOR Y
X_train, y_train = next(train_generator_Y)
real_labels_Y = y_train
real_labels_Y=real_labels_Y.astype(int)
predictions = model_Y.predict(X_train)
print(predictions)
